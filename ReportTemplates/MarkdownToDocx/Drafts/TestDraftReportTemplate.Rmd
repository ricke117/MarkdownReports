...
print(pptx, "TemporaryOutputs/Figures.pptx")
```
...
print(read_docx() %>% 
      ...
      target = "TemporaryOutputs/MSCharts.docx")

block_pour_docx("TemporaryOutputs/MSCharts.docx")
```

```{r, eval = FALSE}
# But that's not interpreted as a figure, so it won't be placed in the table of figures.
# Instead, you can extract the image from a powerpoint containing the image.
# This is not supported for Word docs.
doc <- read_pptx("Images/MSChart.pptx")
  check <- pptx_summary(doc)
                   content, 
